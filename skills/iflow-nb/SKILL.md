...
```bash
IFLOW_DEFAULT_KB="${IFLOW_DEFAULT_KB:-$(cat ~/.config/iflow-nb/default-kb 2>/dev/null)}"
```
...
```bash
IFLOW_KEY="${IFLOW_API_KEY:-$(cat ~/.config/iflow-nb/api-key 2>/dev/null)}"
IFLOW_URL="${IFLOW_BASE_URL:-https://platform.iflow.cn}"
if [ -z "$IFLOW_KEY" ]; then
  echo "缺少 iflow 凭证。请先到 https://iflow.cn/?open=api-key 申请 API Key，然后按 Setup 步骤配置"
  exit 1
fi
```
